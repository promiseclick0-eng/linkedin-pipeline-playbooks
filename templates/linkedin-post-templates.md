...
**How to use:** pick the structure that fits your idea, replace the [brackets], keep the short lines and white space (they make posts skimmable on mobile), and put any external link in the first comment, not the post body.

---

## 1. The Contrarian Take
**Use when:** you disagree with common advice.
```
[Common advice] is wrong.

Here is what actually works:

[Your counter-point, one line.]

[2 to 3 short lines on why, with a specific reason.]

Most people do the opposite. That is the opportunity.
...
```
[A specific scene, one line. A time and place.]

[What happened, 2 to 3 short lines.]

[The turn: what went wrong or surprised you.]
...
```
How to [achieve outcome] (without [the usual pain]):

Step 1: [action]
Step 2: [action]
Step 3: [action]

[One line on the result if they follow it.]
...
```
We took [client type] from [before metric] to [after metric] in [time].
...
Here is why the myth persists: [one line].

And here is what to do instead: [one line].
...
```
Before: [painful old state, 2 to 3 short lines].

After: [the new state].

The one thing that changed: [the lever].
...
```
Most [ICP] struggle with [problem].

Here is the framework we use to fix it:

[Name] = [Step 1] + [Step 2] + [Step 3]

[Two lines explaining the key step.]
...
```
[A specific, slightly contrarian observation about your space, 1 to 2 lines.]
...
[2 to 3 lines of why it is true, with a specific example.]
...
```
[The problem you kept seeing, one line.]

So we built [thing].

It [does the core outcome] so you can [benefit].

[One line of proof or an early result.]
...
Licensed under CC BY 4.0. Use and adapt freely with credit to PromiseClick. See [LICENSE](../LICENSE).
